...
export default function Scalar() {
  ...
  const { handleMatterMatchPress11 } = useHandleClicks();
  const [selectedOption, setSelectedOption] = useState<string | null>(null);

  const handleNext = async () => {
    if (selectedOption) {
      try {
        console.log("Question 10 - selectedOption before saving:", selectedOption); // ADD THIS LINE
        if (selectedOption !== "Mixture") Alert.alert("The correct answer is Mixture.")
        await AsyncStorage.setItem("selectedQuantity10", selectedOption);
  ...
  return (
    <View style={styles.container}>
      ...
      <Image source={require("../utils/pictures/matchmatter/mixture/tea.png")} style={styles.image} />
