...
export default function Scalar() {
  ...
  const { handleMatterMatchPress14 } = useHandleClicks();
  ...
  const handleNext = async () => {
    if (selectedOption) {
      try {
        console.log("Question 13 - selectedOption before saving:", selectedOption); // ADD THIS LINE
        if (selectedOption !== "Solid") Alert.alert("The correct answer is Solid.")
        await AsyncStorage.setItem("selectedQuantity13", selectedOption);
        handleMatterMatchPress14(); // Navigate after saving
      } catch (error) {
  ...
  return (
    <View style={styles.container}>
          ...
          {/* Image */}
          <Image source={require("../utils/pictures/matchmatter/Solid/cabinet.png")} style={styles.image} />
