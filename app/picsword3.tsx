...
export default function PicsWord() {
  ...
  const { handleNext4Pics1WordPress3 } = useHandleClicks();
  ...
  const handleNext = async () => {
    if (selectedOption) {
      try {
        if (selectedOption !== "Pull") Alert.alert("The correct answer is Pull")
        
        await AsyncStorage.setItem("selectedQuantity3", selectedOption);
        handleNext4Pics1WordPress3(); // Navigate after saving
      } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Hint */}
      <Text style={styles.hintText}>Hint: <Text style={styles.hintHighlight}>LPLU</Text></Text>

      {/* Image */}
      <Image source={require("../utils/pictures/.27.jpg")} style={styles.image} />
