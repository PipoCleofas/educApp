...
export default function Scalar() {
  ...
  const { handleMatterMatchPress13 } = useHandleClicks();
  ...
  const handleNext = async () => {
    if (selectedOption) {
      try {
        console.log("Question 12 - selectedOption before saving:", selectedOption); // ADD THIS LINE
        if (selectedOption !== "Solid") Alert.alert("The correct answer is Solid.")
        await AsyncStorage.setItem("selectedQuantity12", selectedOption);
        handleMatterMatchPress13(); // Navigate after saving
      } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Image */}
      <Image source={require("../utils/pictures/matchmatter/Solid/table.png")} style={styles.image} />
