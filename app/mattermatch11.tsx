...
export default function Scalar() {
  ...
  const { handleMatterMatchPress12 } = useHandleClicks();
  const [selectedOption, setSelectedOption] = useState<string | null>(null);

  const handleNext = async () => {
    if (selectedOption) {
      try {
        console.log("Question 11 - selectedOption before saving:", selectedOption); // ADD THIS LINE
        if (selectedOption !== "Solid") Alert.alert("The correct answer is Solid.")
        await AsyncStorage.setItem("selectedQuantity11", selectedOption);
  ...
  return (
    <View style={styles.container}>
      ...
      <Image source={require("../utils/pictures/matchmatter/Solid/chair.png")} style={styles.image} />
