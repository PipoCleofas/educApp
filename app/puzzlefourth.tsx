...
const PuzzleFourth = () => {
  ...
  const handleSubmit = async () => {
    const correctAnswer = "reverse fault";
    const userAnswer = answer.trim().toLowerCase();
    const isCorrect = userAnswer === correctAnswer;
    
    console.log("--- Puzzle 4 Debug ---");
    console.log("User answer:", userAnswer);
    console.log("Expected answer:", correctAnswer);
    console.log("Is correct?", isCorrect);
    
    try {
      await AsyncStorage.setItem("puzzleCorrect4", isCorrect.toString());
      console.log("Saved puzzleCorrect4:", isCorrect);
      
      const storedValue = await AsyncStorage.getItem("puzzleCorrect4");
      console.log("Retrieved puzzleCorrect4:", storedValue);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.questionContainer}>
        <Text style={styles.scrambledText}>SEEVRRE LUAFT</Text>
        <Text style={styles.questionText}>
          Result from compressional{"\n"}
          stress. The hanging wall{"\n"}
          moves upward (e.g.,
          convergent boundaries, like{"\n"}
          the Himalayas).
        </Text>
